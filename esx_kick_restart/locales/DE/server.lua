...
RegisterCommand("kickall", function()
		RegisterServerEvent("kickall")
			...
			if grupos.group ~= 'user' then
				DropPlayer(_source, 'Das Roleplay wurde automatisch beendet. Dein Fortschritt wurde gespeichert. Grund: Server neu start.')		
			end
		end)
	end)
...
RegisterCommand("closeconnect", function()
	...
	if grupos.group ~= 'user' then
		...
		DropPlayer(_source, 'Kick Grund: Der Server startet gerade Neu!.') --message will work soon
